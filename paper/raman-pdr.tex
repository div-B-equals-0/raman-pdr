...
\begin{table}
  ...
  \begin{tabular}{r r R R R R}\toprule
    ...
    & Line or Band & x_0, \si{mpc} & \delta x, \si{mpc} & I(\text{Peak}) & I(\text{BG}) \\
    (1) & (2) & (3) & (4) & (5) & (6) \\
    \midrule
    \hii{} region emission lines
    & [\ion{O}{3}] \(\lambda 5007\) & -32.1 & 45 & 13.059 & 11.642 \pm 5.605 \\
    Peak and BG: \(\int I_\lambda \, d\lambda\), \SI{e-6}{erg.s^{-1}.cm^{-2}.sr^{-1}}
    & [\ion{O}{2}] \(\lambda 7318\) & -9.4 & 25 & 0.882 & 0.232 \pm 0.026 \\    
    & [\ion{O}{1}] \(\lambda 6300\) & -1.7 & 9 & 0.152 & 0.045 \pm 0.001 \\     
    & \ion{O}{1}   \(\lambda 8446\) & 0.0 & 8 & 0.226 & 0.071 \pm 0.000 \\      
    \addlinespace[4pt]
    Raman bands
    & B033 & 0.8 & 20 & 0.455 & 0.858 \pm 0.007 \\
    Peak and BG: \(I_\lambda\), \SI{e-9}{erg.s^{-1}.cm^{-2}.sr^{-1}.\angstrom^{-1}} 
    & R040 & 1.8 & 18 & 0.480 & 0.815 \pm 0.023 \\
    & B054 & 5.0 & 9 & 0.233 & 0.641 \pm 0.009 \\
    & R058 & 5.5 & 11 & 0.285 & 0.591 \pm 0.017 \\
    & B080 & 7.1 & 11 & 0.136 & 0.437 \pm 0.013 \\
    & R087 & 7.1 & 15 & 0.178 & 0.393 \pm 0.017 \\
    & B133 & 6.7 & 10 & 0.128 & 0.302 \pm 0.004 \\
    & R136 & 3.1 & 27 & 0.072 & 0.198 \pm 0.007 \\
    \addlinespace[4pt]
    IRAC bands
    & \SI{3.6}{\micron} & 11.6 & 21 & 0.200 & 0.182 \pm 0.018 \\
    Peak and BG: \(I_\nu\), \si{GJy.sr^{-1}}
    & \SI{5.8}{\micron} & 12.8 & 23 & 1.702 & 0.966 \pm 0.073 \\
    & \SI{8.0}{\micron} & 13.9 & 24 & 4.866 & 2.622 \pm 0.119 \\
    \addlinespace[4pt]
    PDR emission lines
    & \chem{H_2} \SI{2.21}{\micron} & 23.6 & 19 & 0.169 & 0.997 \pm 0.002 \\
    Peak and BG: \(\int I_\lambda \, d\lambda\), median normalized
    & \chem{^{12}CO} (19--18) & 33.8 & 32 & 3.930 & 0.399 \pm 0.001 \\
    & [\ion{C}{1}] \(\lambda 8727\) & 37.7 & 30 & 0.495 & 0.854 \pm 0.122 \\
    & \chem{^{12}CO} (1--0) & 41.1 & 44 & 0.472 & 0.746 \pm 0.173 \\
    & \chem{^{13}CO} (1--0) & 49.8 & 33 & 0.891 & 0.823 \pm 0.112 \\
    \bottomrule
    \multicolumn{6}{p{14cm}}{\textsc{Note}---Results of fitting a single Gaussian plus linear background to the brightness profiles shown in Figure~\ref{fig:raman-bar-profile} (see figure caption for references).  Columns: (1)~Type of tracer and surface brightness units for the values given in columns~5 and~6. (2)~Specific emission line or continuum band. (3)~Position of fitted Gaussian relative to ionization front. (4)~FWHM of fitted Gaussian.  (5)~Peak intensity of fitted Gaussian, with brightness units given in column~1. (6)~Intensity of fitted background at position \(x_0 \pm \delta x\).}
  \end{tabular}
\end{table}
...
the right.  Table~\ref{tab:bar-profile-peaks} shows the positions,
widths, and intensities of the peaks in each tracer, as determined by
fitting a single Gaussian plus a linear background.  
...
[\ion{O}{1}], and \ion{O}{1} lines.  The peak in [\ion{O}{1}]
...
front, as discussed in detail in \S~\ref{sec:structure-orion-bar}
below.

Unlike the optical emission lines, the Raman wing bands (upper panel
of Fig.~\ref{fig:raman-bar-profile}) all peak at positive values of
\(x\), corresponding to fully neutral gas in the PDR.\@ There is a
very close correspondence between corresponding pairs of blue and red
...
neutral gas as one moves farther from the \ha{} line center, which is
quantitatively confirmed by the Gaussian fits in
Table~\ref{tab:bar-profile-peaks}.  The peak displacement from the
ionization front varies approximately as 
\(x_0 \propto 1 / \langle \sigma_\lambda \rangle\), where the \lyb{} wing cross section
\(\langle \sigma_\lambda \rangle\) that feeds each band is given in column~6 of
Table~\ref{tab:wav-bands}.

\begin{figure}
  \centering
  \includegraphics[width=0.7\linewidth]{figs/raman-band-displacements-plot}
  \caption{Displacement from the ionization front of Raman emission
    peak in each band, plotted as a function of the cross-section of
    the \lyb{} wing.  Color scheme is as in
    Fig.~\ref{fig:raman-spectra-1d}.  Black lines show a simple model
    in which the peaks occur at \(\tau = 1\) for 3 different constant
    neutral hydrogen densities, as indicated in the legend. }
