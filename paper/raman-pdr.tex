...
\begin{figure}
  ...
  \caption{Spatial location of extracted regions}
...
nebula where line emission and absorption cancel out so that
\(\Ttb(v) \approx 0\).  Absorption component~M is identified by \vdw{} as
associated with \chem{H^0} in the Bar, due to its velocity and spatial
distribution.  Component~M consists of a string of knots with
...
distances from the ionization front \ion{H}{1} at this velocity is
...
where \(\Ttb(v) = 0\) occurs between these two at \SI{+0.012}{pc} where
\(\Tc \approx \SI{1500}{K}\).
