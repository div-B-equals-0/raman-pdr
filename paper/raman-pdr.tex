...
Section~\ref{sec:broad-band-raman} presents a discussion of
how the spatial and spectral
characteristics of the broad \ha{} wings can be used to diagnose
the physical conditions in the PDR, while
section~\ref{sec:absorpt-feat-raman} considers 
what extra information
can be obtained from the absorption features that are superimposed on the
...
In step~1, radiative transfer in the FUV band determines the mean FUV intensity,
\(J_\lambda(\lambda_1)\), at some point in the nebula.
In step~2, the scattering itself (see Appendix~\ref{sec:raman-theory})
determines a visual-band emissivity \(j_\lambda(\lambda_2)\)
...
For step~2, assuming that the scattering is isotropic, the visual emissivity is simply
\begin{equation}
  \label{eq:raman-emissivity}
  j_\lambda (\lambda_2) = \frac{\lambda_1}{\lambda_2} J_\lambda(\lambda_1) \, n(\chem{H^0}) \, \sigma(\lambda_1) \, f_{\ha}(\lambda_1) ,
\end{equation}
...
The factor \(\lambda_1 / \lambda_2\) accounts in the reduction in energy per photon when passing from
the FUV to visual bands.

The first question to consider with the Raman scattering is whether it is
optically thick or optically thin.
If it is optically thin, then the scattered intensity in the \ha{} wings
can be approximated as
\begin{equation}
  \label{eq:thin-scattered-intensity}
  I_\lambda (\lambda_2) = \frac{\lambda_1}{\lambda_2} \int_{\text{los}}
  J_\lambda(\lambda_1) n(\chem{H_0}) \sigma(\lambda_1) f_{\ha}(\lambda_1) \, dz
\end{equation}
...
\subsection{Raman scattering as a density diagnostic of the shallow PDR}
\label{sec:raman-scattering-as}

If the peak emission occurs for a
...
\(\langle \sigma_\lambda \rangle < \sigma_{\text{dust}}\).  
...
\subsection{The failure of rival explanations for the 6633, 6664~\AA{} lines}
...
\subsubsection{Why they cannot be visual-band neutral/molecular absorption lines from the PDR}
...
The same argument as in the previous section also rules out any possibility
