...
#+begin_src python :return figfile :results file
  ...
  for wave, label in strong_lines.items():
      ax.axvline(wave, 0.22, 1.0, color="k", lw=2, zorder=-5)
      ax.text(wave, 0.79, label, ha="center", va="top", rotation="vertical")
  ...
  for wave, label in absorption_lines.items():
      ...
      miny = 0.25 if "DIB" in label else 0.24
      ytext = 1.38 if "DIB" in label else 0.78
      if "5801" in label:
          dwav = -2.0
      elif "5812" in label:
          dwav = 2.0
