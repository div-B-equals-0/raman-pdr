...
+ [N II] 6527
+ [Ni III] 6533
**** TODO Concatenate wavsec 2 and wavsec 3
...
***** TODO Now we need to do this on the server for the full res cubes
...
: extract-raman-bands.py
:           1,199 100%    0.00kB/s    0:00:00            1,199 100%    0.00kB/s    0:00:00 (xfr#1, to-chk=2/4)
: 
: sent 578 bytes  received 46 bytes  249.60 bytes/sec
: total size is 4,875  speedup is 7.81
...
#+begin_src python :return figfile :results file
  ...
  ax.set(
      ...
      ylabel="Relative Flux, $F_{\lambda} / F_{6400}$ + offset",
      # xlim=[wavmin, wavmax],
      ylim=[0.55, 2.25],
  )
  ax.minorticks_on()
...
#+header: :var STRETCH=0.3 Q=4 STARS=star-coords
#+begin_src python :results file :return figfile
  import numpy as np
  import matplotlib.pyplot as plt
  from astropy.visualization import make_lupton_rgb
  ...
  w = WCS(fits.open(f"{DATADIR}/{rf}")["SCALED"].header)
...
#+header: :var SCALE="sqrt" STARS=star-coords
#+begin_src python :results file :return figfile
  import numpy as np
  import matplotlib.pyplot as plt
  from astropy.visualization import make_lupton_rgb
...
#+call: bar-profile(DATAPATH="../../dust-wave-case-studies/data/LP-Ori-Spitzer/atlas/irsa.ipac.caltech.edu/data/SPITZER/Enhanced/SEIP/images/4/0044/40044081/2/40044081-82", INFILE="40044081.40044081-82-short.IRAC.1.mosaic.fits", HDU=0, NBLOCK=10, NORM="no")

#+RESULTS:
[[file:40044081.40044081-82-short.IRAC.1.mosaic-bar-profile.jpg]]

#+call: bar-profile(DATAPATH="../../dust-wave-case-studies/data/LP-Ori-Spitzer/atlas/irsa.ipac.caltech.edu/data/SPITZER/Enhanced/SEIP/images/4/0044/40044081/2/40044081-82", INFILE="40044081.40044081-82-short.IRAC.4.mosaic.fits", HDU=0, NBLOCK=100, NORM="no")

#+RESULTS:
[[file:40044081.40044081-82-short.IRAC.4.mosaic-bar-profile.jpg]]
...
#+begin_src python :return figfile :results file
  ...
  molecules = [
      ["mom0_48_57_12co_pix_2_Tmb", 3.0, "$^12$CO"],
      #["40044081.40044081-82-short.IRAC.4.mosaic", 1.0, "MIR $5.8 \mu$m"],
  ]

  for lineid, offset, label in emlines:
      x, y = load_data(f"linesum-{lineid}-bin001-bar-profile.json")
      m = x < 30
      ax.plot(x[m], y[m] + offset, color="k", lw=0.5)

  for lineid, scale, label in molecules:
      x, y = load_data(f"{lineid}-bar-profile.json")
      m = x > 15
      ax.plot(x[m], y[m]*scale, color="k", lw=2, alpha=0.3)


  raman_fmt = "muse-hr-data-ha-raman-{}-cont-sub-multibin-1000-bar-profile.json"
  for blue, red, alpha, cb, cr in raman_bands:
      x, y = load_data(raman_fmt.format(blue)) 
      ax2.plot(x, y, color=cb, alpha=alpha, lw=2)
      x, y = load_data(raman_fmt.format(red)) 
      ax2.plot(x, y, color=cr, alpha=alpha, lw=2)
  ...
  # 40044081.40044081-82-short.IRAC.1.mosaic-bar-profile.json
  # 40044081.40044081-82-short.IRAC.4.mosaic-bar-profile.json
  # continuum-N_III-6634-bin001-bar-profile.json
  # linesum-Ar_III-7136-bin001-bar-profile.json
  # linesum-S_III-9069-bin001-bar-profile.json
  # mom0_48_57_12co_pix_2_Tmb-bar-profile.json
  # muse-derived-Ne-iii-multibin-SN0003-bar-profile.json
  # muse-derived-Ne-multibin-SN0003-bar-profile.json
  # muse-derived-Te-iii-multibin-SN0003-bar-profile.json
  # muse-derived-Te-multibin-SN0003-bar-profile.json
  # muse-hr-data-ha-raman-B033-cont-sub-multibin-1000-bar-profile.json
  # muse-hr-data-ha-raman-B054-cont-sub-multibin-1000-bar-profile.json
  # muse-hr-data-ha-raman-B080-cont-sub-multibin-1000-bar-profile.json
  # muse-hr-data-ha-raman-B133-cont-sub-multibin-1000-bar-profile.json
  # muse-hr-data-ha-raman-R040-cont-sub-multibin-1000-bar-profile.json
  # muse-hr-data-ha-raman-R058-cont-sub-multibin-1000-bar-profile.json
  # muse-hr-data-ha-raman-R087-cont-sub-multibin-1000-bar-profile.json
  # muse-hr-data-ha-raman-R136-cont-sub-multibin-1000-bar-profile.json
  # muse-hr-data-ha-raman-TOTAL-multibin-1000-bar-profile.json
