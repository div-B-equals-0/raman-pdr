...
#+call: bar-profile(INFILE="linesum-blue-C_I-8727-multibin-500.fits", NBLOCK=1000)
...
#+begin_src python :return figfile :results file
  ...
  molecules = [
      ["mom0_87_109_13co_pix_2_Tmb", 4.0, 1, "$^{13}$CO (1-0)"],
      ["mom0_48_57_12co_pix_2_Tmb", 3.0, 2, "$^{12}$CO (1-0)"],
      ["linesum-blue-C_I-8727-multibin-500", 2.7, 3, r"[C I] $\lambda 8727$"],
  ...
  for lineid, scale, lw, label in molecules:
      x, y = load_data(f"{lineid}-bar-profile.json")
      m = x > 10
      ax.plot(x[m], y[m]*scale, color="k", lw=lw, alpha=0.3)
      x0 = 75
      i0 = np.sum(x < x0)
      y0 = y[i0]
      ax.text(x0, scale*y0,
              f"{label}",
              bbox=whitebox, ha="center", va="center", fontsize="small")
  ax.text(75, 11.0, "PDR emission lines", ha="center", fontsize="small")
...
** [1/4] Brightness profiles of PDR emission
+ [X] I have the CO from the Kong:2018a data
+ [ ] I can get H_2 from HAWKI
+ [ ] I can get dust/PAH from IRAC
  + Currently there is a problem with NaNs in the median profile
+ [ ] I can get [C I] from MUSE
