...
+ Note we can't have ~H2   2.12125m~ since we don't have the big molecule
  + So we have the simple versin ~H2 v 0~
...
+ [ ] But we need to add in the Rayleigh/Raman scattering as well
#+name: cloudy-fuv-spec-vs-colden-plot
#+header: :var PREFIX="trap-bar" SUFFIX=""
#+begin_src python :return figfile :results file
  import sys
  import numpy as np
  from matplotlib import pyplot as plt
  ...
  for i, colden_string in enumerate(colden_list):
      ...
      norm = np.median(Flam_inc[mask])
      if i == 0:
          ax.plot(wav, Flam_inc/norm, c="k", lw=1, label="Incident")
      ax.fill_between(wav, Flam_trans/norm, fc="r", ec="none", alpha=0.3)
...
[[file:cloudy-fuv-spec-vs-colden-trap-bar.pdf]]

#+call: cloudy-fuv-spec-vs-colden-plot("trap-bar", "-full-dust")
...
#+begin_src python :return figfile :results file
  ...
  labels = tau.colnames[1:]


  for label in labels:
      ax.plot(N_H, tau[label], label=label)

  ax.legend(ncol=5, fontsize="x-small", loc="bottom")
  ax.set(
      xscale = "log",
      yscale = "log",
      xlim = [0.0, None],
