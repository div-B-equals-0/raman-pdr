...
#+begin_src python :return figfile :results file
  ...
  molecules = [
      ...
      ["Orion-Bar-H2-minus-060-Brg", -12.7, 13.0, 3.0, r"H$_{2}$ 2.21$\,\mu$m"],
  ]
...
|------------------+-------+------+----------+----------+---------|
| $3.6\, \mu$m       |  11.6 |   21 |    0.200 |    0.182 |   0.018 |
| $5.8\, \mu$m       |  12.8 |   23 |    1.702 |    0.966 |   0.073 |
| $8.0\, \mu$m       |  13.9 |   24 |    4.866 |    2.622 |   0.119 |
|------------------+-------+------+----------+----------+---------|
| H$_{2}$ 2.21$\,\mu$m  |  23.6 |   19 |    0.169 |    0.997 |   0.002 |
| $^{12}$CO (19-18)   |  33.8 |   32 |    3.930 |    0.399 |   0.001 |
| [C I] $\lambda 8727$   |  37.7 |   30 |    0.495 |    0.854 |   0.122 |
| $^{12}$CO (1-0)     |  41.1 |   44 |    0.472 |    0.746 |   0.173 |
| $^{13}$CO (1-0)     |  49.8 |   33 |    0.891 |    0.823 |   0.112 |
|------------------+-------+------+----------+----------+---------|
...
| Band |  x0 | sigma | 1/sigma |
|------+-----+-------+---------|
| B033 | 0.8 | 2.069 |   0.483 |
| R040 | 1.8 | 1.451 |   0.689 |
| B054 | 5.0 | 0.802 |   1.247 |
| R058 | 5.5 | 0.695 |   1.439 |
| B080 | 7.1 | 0.378 |   2.646 |
| R087 | 7.1 | 0.299 |   3.344 |
| B133 | 6.7 | 0.144 |   6.944 |
| R136 | 3.1 | 0.119 |   8.403 |
#+TBLFM: $4=1/$-1;f3
...
#+begin_src python :results file :return figfile :var INTAB=raman-band-displacements
  ...
  x0s, sigmas = [], []
  for band, x0, sigma, _ in INTAB:
      ax.scatter(sigma, x0, c=next(colors), alpha=next(alphas), s=200)
      dy = -14 if band.startswith("R") else -4
      ax.annotate(band, xy=(sigma, x0), xytext=(dy, 8), textcoords="offset points", fontsize="x-small")
      x0s.append(x0)
      sigmas.append(sigma)
  x0s = np.array(x0s)
