...
#+begin_src python :return figfile :results file
  ...
  molecules = [
      ...
      ["Orion-Bar-H2-minus-060-Brg", -14.1, 13.0, 3.0, r"H$_{2}$ 2.21$\,\mu$m"],
  ]
  ...
  ax.text(x0, 15.0, "Optical emission lines", ha="center", fontsize="small")
...
#+begin_src python :return results
  ...
  class Results:
      ...
      def __init__(self):
          self.table = [["Label", "Peak", "x", "FWHM", "Base"], None]
      def update(self, label, g):
          self.table.append([label,
                             np.round(g.amplitude_0, 3),
                             np.round(g.mean_0, 1),
                             np.round(2.3*g.stddev_0, 0),
                             np.round(g.intercept_1, 3),
          ])
      def hline(self):
  ...
  for blue, red, alpha, cb, cr, wav in raman_bands:
      ...
      results.update(blue, g)
      m = (xr > -90) & (xr < 60)
      g = fit_gaussian(xr[m], yr[m])
      results.update(red, g)
  results.hline()
...
#+RESULTS:
| Label            |     Peak |     x | FWHM |     Base |
|------------------+----------+-------+------+----------|
| [O III] $\lambda 5007$ |    1.355 | -32.1 | 45.0 |    0.791 |
| [O II] $\lambda 7318$  |    3.689 |  -9.4 | 25.0 |     0.93 |
| [O I] $\lambda 6300$   |    4.237 |  -1.7 |  9.0 |    1.251 |
| O I $\lambda 8446$     |     3.91 |   0.0 |  8.0 |    1.231 |
|------------------+----------+-------+------+----------|
| B033             |    0.455 |   0.8 | 20.0 |    0.858 |
| R040             |     0.48 |   1.8 | 18.0 |    0.813 |
| B054             |    0.233 |   5.0 |  9.0 |    0.636 |
| R058             |    0.285 |   5.5 | 11.0 |    0.582 |
| B080             |    0.136 |   7.1 | 11.0 |    0.429 |
| R087             |    0.178 |   7.1 | 15.0 |    0.385 |
| B133             |    0.128 |   6.7 | 10.0 |    0.299 |
| R136             |    0.072 |   3.1 | 27.0 |    0.197 |
|------------------+----------+-------+------+----------|
| $3.6\, \mu$m       |  200.014 |  11.6 | 21.0 |  192.254 |
| $5.8\, \mu$m       | 1701.579 |  12.8 | 23.0 |  1007.22 |
| $8.0\, \mu$m       | 4865.801 |  13.9 | 24.0 | 2689.414 |
|------------------+----------+-------+------+----------|
| H$_{2}$ 2.21$\,\mu$m  |    0.191 |  23.6 | 19.0 |    1.132 |
| $^{12}$CO (19-18)   |    7.525 |  33.8 | 32.0 |    0.762 |
| [C I] $\lambda 8727$   |    0.532 |  37.7 | 30.0 |    1.081 |
| $^{12}$CO (1-0)     |    0.755 |  41.1 | 44.0 |    0.934 |
| $^{13}$CO (1-0)     |    1.335 |  49.8 | 33.0 |    0.979 |
|------------------+----------+-------+------+----------|
| Ne               | 1426.813 |  -8.8 | 27.0 | 1600.395 |
| Ne-iii           | 1990.752 | -15.0 | 26.0 | 3032.843 |
| Te               |  377.649 |  -2.2 | 15.0 | 8866.384 |
| Te-iii           |  642.275 |  -6.6 | 25.0 | 7967.247 |
|------------------+----------+-------+------+----------|

**** Graph of structure of the Bar
...
+ H_2 2.12 micron line
  + Walmsley:2010a is what everyone uses, but this is a restricted region
...
+ First message
  #+begin_example
    He estado viendo el ruido en la zona sin emisión cercana a la posición y es de 2 mJy (más del doble del ruido en el centro de la imagen, tal como esperamos). Te adjunto una imagen donde se ve una fuente a 1.3 mm que está muy cerca de la cruz roja (posición que me has pasado), y esa fuente está a 11.1 mJy, con lo cual está a un S/N ~ 5.5 (los contornos en la figura corresponden a 6 y 9 mJy).
   
    Espero que esto te sirva :) Cualquier otra cosa en que pueda ayudar me dices…
  #+end_example
+ Second message
  #+begin_example
    Sí, hay una fuente pero solo se detecta en la configuración compacta+extendida, y no en la extendida. Esto implica que la fuente NO es muy compacta. He revisado y con la config extendida, en la misma zona, tengo un ruido de 1 mJy. Por tanto, si esa fuente fuera muy compacta, la tendría que haber detectado con la configuración extendida con una S/N ~ 11, y no es el caso.

    Tal como se dice en la Sección 2.1 del artículo, con la configuración extendida se cubrieron líneas de base de 33 a 1230 klambda. Esto implica que en esa configuración estamos filtrando estructuras con tamaños >~ 3''. En cambio, con la config compacta filtramos estructuras con tamaños >~ 6''. De ahí se puede decir que la fuente que detectamos con configuración compacta+extendida (y que no se detecta en extendida) ha de tener un tamaño de 3''-6'' aproximadamente (1200-2400 AU). Por tanto, uno podría descartar en principio la presencia de un disco. Suponiendo que la temperatura del polvo es de ~ 20 K, 11 mJy corresponden a una masa de gas y polvo de ~ 40 Mjup. Así que parece que es un objeto de muy baja masa y un poco extendido. En caso de tener un núcleo compacto éste ha de tener menos de 15 Mjup (4 veces el ruido con la config extendida). Como no sabemos si el objeto está gravitacionalmente ligado o no, no sabemos el futuro de este objeto... Si estuviera gravitacionalmente ligado, desde luego que sería super-interesante...
  #+end_example
+ Position is roughly
  + 5:35:14.1500 -5:23:40.000
  + 141.5-340.0
+ Nearest sources in Bob's list is
  + 140.4-338.3
    + COUP 602, JW 431, MLLA 336, TCC 1, HC 247, HHH 431, LML 152
  + 137.8-340.0
    + MAX 46, MLLA 328, SB 1, LM 78, HC 242, LML 144, LR 32, PMF 35
  + So it seems it is not a protostellar disk/envelope
    + Taking radius of 2 arcsec = 1.2e16 cm
    + Mass of 40 Mjup = 7.5944e31 g = 0.038 Msun
    + Then \rho = M / (4 pi/3) R**3 = 1.05e-17 g/cm^3 = 3e6 H/cm^3
  + Jeans mass
    + 2 Msun (c_s/0.2 km/s)**3 / sqrt(n /1000) according to Wikipedia
    + \rho c_s^2 = n k T => c_s = sqrt(k T / m) = 0.268 km/s
    + T = 20 K, n = 3e6 => M_J = 0.07 Msun
+ My reply
  #+begin_example
    Muchas gracias por la explicación.  El pico que se ve en el mapa de Hacar también tiene diametro de 4", entonces seguramente están viendo la misma emisión. Todo parece indicar entonces que es solo una nubecita. Si la masa es de 40 Mjup = 0.038 Msun, entonces con un radio de 2" calculo una densidad de 3e6 pcc y una masa de Jeans de 0.07 Msun (si la temperatura es de 20 K), lo cual indica que probablemente no es autogravitante. De hecho, si realmente está asociado con el filamento con el frente de ionización entonces la temperatura podría ser mayor, lo cual va a reducir aun más la masa. 
  #+end_example
+ Pressure comparison
  + Ionization front pressure
    + n = 2e4 pcc, T = 1e4 K
    + P/k = 2 n T = 4e8 cm^-3 K
  + Globule pressure
    + n = 3e6, T = 20 K
    + P/k = 0.6e8 cm^-3 K
  + So thermal pressure of globule is 7 times less than H II region
